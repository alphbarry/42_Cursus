...
**Error**: `Bind for 0.0.0.0:${NGINX_PORT} failed: port is already allocated`

**Solution**:
```bash
# Find process using the port defined in NGINX_PORT
sudo lsof -i :${NGINX_PORT}
# Kill the process or change NGINX_PORT via the helper target (it recreates nginx)
make set-port PORT=3000   # pick any free port
```
