...
- **Port**: Configurable via `NGINX_PORT` (default 443, HTTPS)
...
```
Internet → Nginx (`NGINX_PORT`/HTTPS) → WordPress (PHP-FPM) → MariaDB (3306)
                ↓
...
- If you ever need to change the HTTPS port, run `make set-port PORT=<new value>`; it updates the `.env` file and recreates the Nginx container for you.
...
2. **Access the website:**
   ...
   - Append `:<NGINX_PORT>` to the URL if you changed it from 443
...
- Ensure the port defined in `NGINX_PORT` (default 443) is open in your firewall
- Access via: `https://alphbarr.42.fr` (or your server's IP) and append `:<NGINX_PORT>` if you changed it from 443
- Make sure DNS points to your server's IP address

### Changing the HTTPS Port

1. Run `make set-port PORT=3000` (replace `3000` with your port) to update `.env` and recreate Nginx automatically.
2. Update your firewall/DNS as needed and access the site through `https://<domain>:<PORT>`.
...
wordpress   inception-wordpress  Up              
nginx       inception-nginx      Up              0.0.0.0:${NGINX_PORT}->${NGINX_PORT}/tcp
```
...
echo | openssl s_client -connect alphbarr.42.fr:${NGINX_PORT} -servername alphbarr.42.fr 2>/dev/null | openssl x509 -noout -dates
```
...
**Common causes:**
- Port defined in `NGINX_PORT` already in use: `sudo lsof -i :${NGINX_PORT}`
